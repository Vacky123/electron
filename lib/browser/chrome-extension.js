...
ipcMain.on('CHROME_GET_MESSAGES', function (event, extensionId, language) {
  const pageURL = event.sender._getURL()
  if (isChromeExtension(pageURL)) {
    event.returnValue = getMessages(extensionId, language)
  } else {
    console.error(`Blocked ${pageURL} from calling chrome.i18n.getMessage()`)
